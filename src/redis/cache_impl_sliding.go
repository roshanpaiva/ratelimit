...
func (this *rateLimitCacheImpl) DoSlidingWindowLimit(
	ctx context.Context,
	request *pb.RateLimitRequest,
	limits []*config.RateLimit) []*pb.RateLimitResponse_DescriptorStatus {

	logger.Debugf("starting cache lookup for sliding window")
	...
	for i := 0; i < len(request.Descriptors); i++ {
		cacheKeys[i] = this.generateCacheKey(request.Domain, request.Descriptors[i], limits[i], now, "slidingWindow")
	...
	for i, cacheKey := range cacheKeys {
		...
		expirationSeconds := unitToDivider(limits[i].Limit.Unit, limits[i].Limit.Duration) + unitToSubDivider(limits[i].Limit.Unit, limits[i].Limit.Duration)
	...
	// Now fetch the pipeline and add all requests in past subwindows to determine if within limit.
	responseDescriptorStatuses := make([]*pb.RateLimitResponse_DescriptorStatus,
		len(request.Descriptors))
	for i, cacheKey := range cacheKeys {
		// If no rate limit is specified, we always approve the request.
		if cacheKey.key == "" {
		...
		var limitAfterIncrease uint32 = 0
		if this.perSecondPool != nil && cacheKey.perSecond {
			pipelineFetch(perSecondConn)
		} else {
			pipelineFetch(conn)
		}

		// Fill in the pastKeys slice with the cache keys for each of the past subwindows
		numSubWindows := unitToDivider(limits[i].Limit.Unit, limits[i].Limit.Duration)/unitToSubDivider(limits[i].Limit.Unit, limits[i].Limit.Duration) + 1 // add 1 so that we are more strict
		pastKeys := make([]string, numSubWindows)
		for j := int64(0); j < numSubWindows; j++ {
			subWindowWidth := unitToSubDivider(limits[i].Limit.Unit, limits[i].Limit.Duration)
			windowStart := (now/subWindowWidth)*subWindowWidth - (j * subWindowWidth)
			pastKeys[j] = this.generateCacheKey(request.Domain, request.Descriptors[i], limits[i], windowStart, "slidingWindow").key
		}

		// Call to PipeExec executes "MGET" on past subwindows and returns an array of responses.
		// Each response is the number of requests for that cache key.
		if this.perSecondPool != nil && cacheKey.perSecond {
			val := perSecondConn.PipeExec("MGET", pastKeys)
			ret := val.Array()
			for _, resp := range ret {
				retInt, _ := resp.Int()
				limitAfterIncrease += uint32(retInt)
			}
		} else {
			val := conn.PipeExec("MGET", pastKeys)
			ret := val.Array()
			for _, resp := range ret {
				retInt, _ := resp.Int()
				limitAfterIncrease += uint32(retInt)
		...
		overLimitThreshold := limits[i].Limit.RequestsPerUnit
                limitBeforeIncrease := overLimitThreshold - (limitAfterIncrease - hitsAddend)
		...
		if limitAfterIncrease > overLimitThreshold {
			responseDescriptorStatuses[i] =
				&pb.RateLimitResponse_DescriptorStatus{
					...
					LimitRemaining: limitBeforeIncrease,
				}
			...
			if limitBeforeIncrease >= overLimitThreshold {
				...
				limits[i].Stats.NearLimit.Add(uint64(overLimitThreshold - max(nearLimitThreshold, limitBeforeIncrease)))
                        ...
                        if this.perSecondPool != nil && cacheKey.perSecond {
                          perSecondConn.PipeExec("DECRBY", cacheKey.key, hitsAddend)
                        } else {
                          conn.PipeExec("DECRBY", cacheKey.key, hitsAddend)
                        }
		} else {
			responseDescriptorStatuses[i] =
				&pb.RateLimitResponse_DescriptorStatus{
